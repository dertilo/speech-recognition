...
# stolen from deepspeech.pytorch
...
parser.add_argument("--format", type=str, default="mp3", help="audio format")
...
def prepare_dir(ted_dir):
    ...
    format = args.format
    wav_dir = os.path.join(converted_dir, format)
    ...
    for sph_file in tqdm(entries, total=len(entries)):
        ...
        for utterance_id, utterance in enumerate(all_utterances):
            target_wav_file = os.path.join(wav_dir,
                                           f"{utterance['filename']}_{str(utterance_id)}.{format}")
            target_txt_file = os.path.join(txt_dir,
                                           f"{utterance['filename']}_{str(utterance_id)}.txt")
