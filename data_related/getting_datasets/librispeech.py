...
def _process_file(wav_dir, txt_dir, base_filename, root_dir):
    ...
    subprocess.call(
        [
            "sox {}  -r {} -b 16 -c 1 {}".format(
                full_recording_path, str(args.sample_rate), wav_recording_path
            )
        ],
        shell=True,
    )
    ...
    assert os.path.exists(transcript_file), "Transcript file {} does not exist.".format(
        transcript_file
    )
    transcriptions = open(transcript_file).read().strip().split("\n")
    transcriptions = {
        t.split()[0].split("-")[-1]: " ".join(t.split()[1:]) for t in transcriptions
    }
...
def main():
    ...
    for split_type, lst_libri_urls in LIBRI_SPEECH_URLS.items():
        ...
        if split_type == "train":  # Prune to min/max duration
            create_manifest(
                split_dir,
                "libri_" + split_type + "_manifest.csv",
                args.min_duration,
                args.max_duration,
            )
        else:
