...
if __name__ == "__main__":
    ...
    asr_samples = (
        ASRSample(**s)
        for s in data_io.read_jsonl(
            f"{base_path}/{folder}/manifest.jsonl.gz", limit=100
        )
    )
    id2sample = {s.audio_file: s for s in asr_samples}

    hyps_refs = [
        (d["text"], id2sample[d["id"]].text)
        for d in data_io.read_jsonl(
            "/home/tilo/data/asr_data/GERMAN/results_old_errors.jsonl"
        )
        if "text" in d
    ]
